  ...
  return (
    ...
    >
      <ScrollReveal 
        ...
        delay={index * 100}
        threshold={0.2}
        className={`${isDarkTheme ? 'bg-black' : 'bg-white'} p-6 rounded-xl border ${isDarkTheme ? 'border-white/20 hover:border-white/40' : 'border-black/20 hover:border-black/40'} transition-all duration-300 service-card group-hover:service-shake shadow-sm`}
      >
        <motion.div 
          className={`w-12 h-12 ${isDarkTheme ? 'bg-white/5 text-white' : 'bg-black/5 text-black'} rounded-full flex items-center justify-center mb-4`}
          initial={{ rotate: 0 }}
          whileHover={{ rotate: [0, -10, 10, -5, 5, 0], transition: { duration: 0.5 } }}
        >
          {icon}
        </motion.div>
        <h3 className={`text-xl font-medium mb-2 ${isDarkTheme ? 'text-white group-hover:text-white' : 'text-black group-hover:text-black'} transition-colors duration-300`}>{title}</h3>
        <p className={isDarkTheme ? "text-muted-foreground" : "text-gray-700"}>{description}</p>
      </ScrollReveal>
    </motion.div>
  );
};
...
const Services = () => {
  ...
  return (
    <section id="services" className={`py-24 px-4 relative overflow-hidden ${isDarkTheme ? 'bg-black' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <ScrollReveal 
            animation="slide-down" 
            delay={100} 
            className={`${isDarkTheme ? 'text-white' : 'text-black'} text-sm uppercase tracking-wider mb-2`}
          >
            Our Services
          </ScrollReveal>
          ...
          <ScrollReveal 
            animation="fade-in" 
            delay={200} 
            className={`text-3xl md:text-4xl font-bold mb-4 max-w-3xl mx-auto ${isDarkTheme ? 'text-white' : 'text-black'}`}
          >
